  ...
  return (
    <div className="flex items-center justify-end gap-2">
      <button
        type="button"
        onClick={() => onEdit?.(port)}
        aria-label={`แก้ไข port ${port.port_number}`}
        title="แก้ไข"
      ...
      <button
        ...
        aria-label={`ลบ port ${port.port_number}`}
        title="ลบ"
...
export default function PortTable({
        ...
        render: (port) => {
          ...
          return (
            <button
              ...
              title={port.is_active ? "ปิดใช้งาน" : "เปิดใช้งาน"}
  ...
  return (
    <DataTable
      ...
      emptyText="ไม่พบข้อมูล Port"
      loadingText="กำลังโหลดข้อมูล Port..."
      title="รายการ Port"
      count={ports.length}
    />
  );
